...
def fix_nested_executable(base_game_path: str, relative_exe_path: str) -> str:
    """Create a 0-byte ghost executable in a nested subfolder.

    Handles games like Ryse: Son of Rome that hide their .exe in
    subdirectories (e.g. Bin64/Ryse.exe).  Creates the subfolder tree
    if it does not exist, touches the target file, and makes it
    executable so Steam does not throw a permission error.

    Args:
        base_game_path: Full path to the game's root directory,
            e.g. ``/path/to/steamapps/common/Ryse Son of Rome``.
        relative_exe_path: Path relative to *base_game_path*,
            e.g. ``Bin64/Ryse.exe``.

    Returns:
        Absolute path to the created ghost file.
    """
    target = os.path.join(base_game_path, relative_exe_path)
