...
do_install() {
    ...
    cp "$SRC/logic.py" "$TARGET/"
    cp "$SRC/updater.sh" "$TARGET/"
    cp "$SRC/metadata/version.json" "$TARGET/metadata/"
...
if [ \$EXIT_CODE -ne 0 ]; then
    ...
    echo "\$LAST_LINES" | head -5
    if command -v zenity &>/dev/null; then
        zenity --error --title="Manifest Studio" \
            --text="App crashed (exit code \$EXIT_CODE).\nCheck the log:\n\$LOG" 2>/dev/null || true
    fi
fi
exit \$EXIT_CODE
WRAPPER_EOF
...
case "${1:-}" in
    ...
    version|--version|-v)
        ...
        elif [ -f "$TARGET/metadata/version.json" ]; then
            python3 -c "import json; print(json.load(open('$TARGET/metadata/version.json'))['version'])"
        else
            echo "Not installed"
        fi
        exit 0
...
if [ -f /etc/fedora-release ] || [ -f /etc/redhat-release ] || grep -qi 'fedora\|nobara' /etc/os-release 2>/dev/null; then
    sudo dnf install -y python3-gobject gtk4 libadwaita python3-requests
else
    echo "Warning: not a Fedora/Nobara system. Attempting dnf anyway..."
    sudo dnf install -y python3-gobject gtk4 libadwaita python3-requests || true
fi
